...
public:
	...
	//δικιά μου προσθήκη για uint64 τύπο
	uint64_t GetUnsignedLongLongInteger(std::string section, std::string name, uint64_t default_value) const;
...
//δικιά μου προσθήκη για uint64 τύπο
inline uint64_t INIReader::GetUnsignedLongLongInteger(std::string section, std::string name, uint64_t default_value) const
{
	std::string valstr = Get(section, name, "");
	const char* value = valstr.c_str();
	char* end;
